...
function playSound(sound) {
	var path = 'app/ext/poncho-games-login/resources/audio/' + sound + '.mp3';
...
function playSoundLoop(sound) {
	var path = 'app//resources/audio/' + sound + '.mp3';
	var context = window.audioContext;
	var request = new XMLHttpRequest();
...
class Ghost {
	...
	reset(fullGameReset) {
		...
		this.setDefaultMode();
		this.setMovementStats(this.poncho, this.name, this.level);
	 ...
	 * @param {Object} poncho - poncho's speed is used as the base for the ghosts' speeds
	 * @param {('inky'|'blinky'|'pinky'|'clyde')} name - The name of the current ghost
	 */
	setMovementStats(poncho, name, level) {
		const ponchoSpeed = poncho.velocityPerMs;
		const levelAdjustment = level / 100;

		this.slowSpeed = ponchoSpeed * (0.75 + levelAdjustment);
		this.mediumSpeed = ponchoSpeed * (0.875 + levelAdjustment);
		this.fastSpeed = ponchoSpeed * (1 + levelAdjustment);

		if (!this.defaultSpeed) {
			this.defaultSpeed = this.slowSpeed;
		}

		this.scaredSpeed = ponchoSpeed * 0.5;
		this.transitionSpeed = ponchoSpeed * 0.4;
		this.eyeSpeed = ponchoSpeed * 2;
	...
	setSpriteSheet(name, direction, mode) {
		...
		if (mode === 'scared') {
			this.animationTarget.style.backgroundImage =
				'url(app/ext/poncho-games-login/resources/graphics/' +
	...
	/**
	 * Uses the Pythagorean Theorem to measure the distance between a given postion and poncho
	 * @param {({x: number, y: number})} position - An x-y position on the 2D Maze Array
	 * @param {({x: number, y: number})} poncho - poncho's current x-y position on the 2D Maze Array
	 * @returns {number}
	 */
	calculateDistance(position, poncho) {
		return Math.sqrt(
			(position.x - poncho.x) ** 2 + (position.y - poncho.y) ** 2
		);
	}

	/**
	 * Gets a position a number of spaces in front of poncho's direction
	 * @param {({x: number, y: number})} ponchoGridPosition
	 * @param {number} spaces
	 */
	getPositionInFrontOfponcho(ponchoGridPosition, spaces) {
		const target = Object.assign({}, ponchoGridPosition);
		const pacDirection = this.poncho.direction;
		const propToChange =
	...
	/**
	 * Determines Pinky's target, which is four tiles in front of poncho's direction
	 * @param {({x: number, y: number})} ponchoGridPosition
	 * @returns {({x: number, y: number})}
	 */
	determinePinkyTarget(ponchoGridPosition) {
		return this.getPositionInFrontOfponcho(ponchoGridPosition, 4);
	}

	/**
	 * Determines Inky's target, which is a mirror image of Blinky's position
	 * reflected across a point two tiles in front of poncho's direction.
	 * Example @ app\ext\graphics\spriteSheets\references\inky_target.png
	 * @param {({x: number, y: number})} ponchoGridPosition
	 * @returns {({x: number, y: number})}
	 */
	determineInkyTarget(ponchoGridPosition) {
		const blinkyGridPosition = this.characterUtil.determineGridPosition(
			this.blinky.position,
			this.scaledTileSize
		);
		const pivotPoint = this.getPositionInFrontOfponcho(ponchoGridPosition, 2);
		return {
	...
	/**
	 * Clyde targets poncho when the two are far apart, but retreats to the
	 * lower-left corner when the two are within eight tiles of each other
	 * @param {({x: number, y: number})} gridPosition
	 * @param {({x: number, y: number})} ponchoGridPosition
	 * @returns {({x: number, y: number})}
	 */
	determineClydeTarget(gridPosition, ponchoGridPosition) {
		const distance = this.calculateDistance(gridPosition, ponchoGridPosition);
		return distance > 8 ? ponchoGridPosition : { x: 0, y: 30 };
	}
	 ...
	 * @param {('inky'|'blinky'|'pinky'|'clyde')} name - The name of the current ghost
	 * @param {({x: number, y: number})} gridPosition - The current x-y position on the 2D Maze Array
	 * @param {({x: number, y: number})} ponchoGridPosition - x-y position on the 2D Maze Array
	 * @param {('chase'|'scatter'|'scared'|'eyes')} mode - The character's behavior mode
	 * @returns {({x: number, y: number})}
	 */
	getTarget(name, gridPosition, ponchoGridPosition, mode) {
		// Ghosts return to the ghost-house after eaten
		if (mode === 'eyes') {
			return { x: 13.5, y: 10 };
		}

		// Ghosts run from poncho if scared
		if (mode === 'scared') {
			return ponchoGridPosition;
		}
		...
		if (mode === 'scatter') {
			switch (name) {
				case 'blinky':
					// Blinky will chase poncho, even in Scatter mode, if he's in Cruise Elroy form
					return this.cruiseElroy ? ponchoGridPosition : { x: 27, y: 0 };
				case 'pinky':
					return { x: 0, y: 0 };
				case 'inky':
					return { x: 27, y: 30 };
		...
		switch (name) {
			// Blinky goes after poncho's position
			case 'blinky':
				return ponchoGridPosition;
			case 'pinky':
				return this.determinePinkyTarget(ponchoGridPosition);
			case 'inky':
				return this.determineInkyTarget(ponchoGridPosition);
			case 'clyde':
				return this.determineClydeTarget(gridPosition, ponchoGridPosition);
			default:
				// TODO: Other ghosts
				return ponchoGridPosition;
		}
	...
	determineBestMove(
		...
		let bestMove;
		const target = this.getTarget(name, gridPosition, ponchoGridPosition, mode);
	 ...
	 * @returns {({ top: number, left: number})}
	 */
	handleSnappedMovement(elapsedMs, gridPosition, velocity, ponchoGridPosition) {
		const newPosition = Object.assign({}, this.position);
	...
	handleMovement(elapsedMs) {
		...
		const ponchoGridPosition = this.characterUtil.determineGridPosition(
			this.poncho.position,
			this.scaledTileSize
		);
		...
		this.checkCollision(gridPosition, ponchoGridPosition);
	...
	/**
	 * Checks if the ghost contacts poncho - starts the death sequence if so
	 * @param {({x: number, y: number})} position - An x-y position on the 2D Maze Array
	 * @param {({x: number, y: number})} poncho - poncho's current x-y position on the 2D Maze Array
	 */
	checkCollision(position, poncho) {
		if (
			this.calculateDistance(position, poncho) < 1 &&
			this.mode !== 'eyes' &&
			this.allowCollision
		) {
			if (this.mode === 'scared') {
...
class poncho {
	constructor(scaledTileSize, mazeArray, characterUtil) {
		...
		this.mazeArray = mazeArray;
		this.characterUtil = characterUtil;
		this.animationTarget = document.getElementById('poncho');
		this.ponchoArrow = document.getElementById('poncho-arrow');
	...
	reset() {
		...
		this.ponchoArrow.style.backgroundImage =
			'url(app/ext/poncho-games-login/resources/graphics/' +
			`spriteSheets/characters/poncho/arrow_${this.direction}.svg)`;
	}

	/**
	 * Sets various properties related to poncho's movement
	 * @param {number} scaledTileSize - The dimensions of a single tile
	 */
	...
	/**
	 * Sets values pertaining to poncho's spritesheet animation
	 */
	setSpriteAnimationStats() {
	...
	/**
	 * Sets css property values for poncho and poncho's Arrow
	 * @param {number} scaledTileSize - The dimensions of a single tile
	 * @param {number} spriteFrames - The number of frames in poncho's spritesheet
	 */
	setStyleMeasurements(scaledTileSize, spriteFrames) {
		this.measurement = scaledTileSize * 2;
		...
		this.ponchoArrow.style.height = `${this.measurement * 2}px`;
		this.ponchoArrow.style.width = `${this.measurement * 2}px`;
		this.ponchoArrow.style.backgroundSize = `${this.measurement * 2}px`;
	}

	/**
	 * Sets the default position and direction for poncho at the game's start
	 * @param {number} scaledTileSize - The dimensions of a single tile
	 */
	...
	/**
	 * Calculates how fast poncho should move in a millisecond
	 * @param {number} scaledTileSize - The dimensions of a single tile
	 */
	calculateVelocityPerMs(scaledTileSize) {
		// In the original game, poncho moved at 11 tiles per second.
	...
	setSpriteSheet(direction) {
		this.animationTarget.style.backgroundImage =
			'url(app/ext/poncho-games-login/resources/graphics/' +
			`spriteSheets/characters/poncho/poncho_${direction}.svg)`;
	}
	...
	prepDeathAnimation() {
		...
		this.animationTarget.style.backgroundSize = `${bgSize}px`;
		this.animationTarget.style.backgroundImage =
			'url(app/ext/poncho-games-login/resources/' +
			'graphics/spriteSheets/characters/poncho/poncho_death.svg)';
		this.animationTarget.style.backgroundPosition = '0px 0px';
		this.ponchoArrow.style.backgroundImage = '';
	}

	/**
	 * Changes poncho's desiredDirection, updates the ponchoArrow sprite, and sets moving to true
	 * @param {Event} e - The keydown event to evaluate
	 * @param {Boolean} startMoving - If true, poncho will move upon key press
	 */
	changeDirection(newDirection, startMoving) {
		this.desiredDirection = newDirection;
		this.ponchoArrow.style.backgroundImage =
			'url(app/ext/poncho-games-login/resources/graphics/' +
			`spriteSheets/characters/poncho/arrow_${this.desiredDirection}.svg)`;
	...
	/**
	 * Updates the position of the leading arrow in front of poncho
	 * @param {({top: number, left: number})} position - poncho's position during the current frame
	 * @param {number} scaledTileSize - The dimensions of a single tile
	 */
	updateponchoArrowPosition(position, scaledTileSize) {
		this.ponchoArrow.style.top = `${position.top - scaledTileSize}px`;
		this.ponchoArrow.style.left = `${position.left - scaledTileSize}px`;
	}

	/**
	 * Handle poncho's movement when he is snapped to the x-y grid of the Maze Array
	 * @param {number} elapsedMs - The amount of MS that have passed since the last update
	 * @returns {({ top: number, left: number})}
	 */
	...
	/**
	 * Handle poncho's movement when he is inbetween tiles on the x-y grid of the Maze Array
	 * @param {({x: number, y: number})} gridPosition - x-y position during the current frame
	 * @param {number} elapsedMs - The amount of MS that have passed since the last update
	 * @returns {({ top: number, left: number})}
	 */
	...
	draw(interp) {
		...
		this.animationTarget.style.visibility = this.display
			? this.characterUtil.checkForStutter(this.position, this.oldPosition)
			: 'hidden';
		this.ponchoArrow.style.visibility = this.animationTarget.style.visibility;

		this.updateponchoArrowPosition(this.position, this.scaledTileSize);
	...
	/**
	 * Handles movement logic for poncho
	 * @param {number} elapsedMs - The amount of MS that have passed since the last update
	 */
	update(elapsedMs) {
		this.oldPosition = Object.assign({}, this.position);
...
class GameCoordinator {
	...
	async showGameLoader() {
		const loadingContainer = document.getElementById('loading-container');
		loadingContainer.style.opacity = 1;

		const loadingponcho = document.getElementById('loading-poncho');
		const containerWidth =
			loadingContainer.scrollWidth - loadingponcho.scrollWidth;
		const loadingDotMask = document.getElementById('loading-dot-mask');
		loadingponcho.style.left = '0';
		loadingDotMask.style.width = '0';

		return new Promise((resolve) => {
			setTimeout(() => {
				loadingponcho.style.left = `${containerWidth}px`;
				loadingDotMask.style.width = loadingponcho.style.left;
				setTimeout(() => {
	...
	preloadAssets() {
		return new Promise((resolve) => {
			...
			const imgBase = 'app/ext/poncho-games-login/resources/graphics/spriteSheets/';
			const imgSources = [
				// poncho
				`${imgBase}characters/poncho/arrow_down.svg`,
				`${imgBase}characters/poncho/arrow_left.svg`,
				`${imgBase}characters/poncho/arrow_right.svg`,
				`${imgBase}characters/poncho/arrow_up.svg`,
				`${imgBase}characters/poncho/poncho_death.svg`,
				`${imgBase}characters/poncho/poncho_error.svg`,
				`${imgBase}characters/poncho/poncho_down.svg`,
				`${imgBase}characters/poncho/poncho_left.svg`,
				`${imgBase}characters/poncho/poncho_right.svg`,
				`${imgBase}characters/poncho/poncho_up.svg`,
				...
				// Misc
				'app/ext/poncho-games-login/resources/graphics/extra_life.png'
			];

			const audioBase = 'app/ext/poncho-games-login/resources/audio/';
			const audioSources = [
		...
		return new Promise((resolve, reject) => {
			...
			sources.forEach((source) => {
				...
				const elementReady = () => {
					...
					// loadingponcho.style.left = `${percent * containerWidth}px`;
					// loadingDotMask.style.width = loadingponcho.style.left;
	...
	reset() {
		...
		this.allowponchoMovement = false;
	...
	/**
	 * Loop which periodically checks which pickups are nearby poncho.
	 * Pickups which are far away will not be considered for collision detection.
	 */
	collisionDetectionLoop() {
		if (this.poncho.position) {
			const maxDistance = this.poncho.velocityPerMs * 750;
			const ponchoCenter = {
				x: this.poncho.position.left + this.scaledTileSize,
				y: this.poncho.position.top + this.scaledTileSize
			};
			...
			this.pickups.forEach((pickup) => {
				pickup.checkponchoProximity(maxDistance, ponchoCenter, debugging);
			});
		}
	}

	/**
	 * Displays "Ready!" and allows poncho to move after a breif delay
	...
	startGameplay(initialStart) {
		...
		new Timer(() => {
			...
			this.allowponchoMovement = true;
			this.poncho.moving = true;
	...
	updateExtraLivesDisplay() {
		...
		for (let i = 0; i < this.lives; i += 1) {
			...
			extraLifePic.setAttribute('src', 'app/ext/poncho-games-login/resources/graphics/extra_life.svg');
	...
	/**
	 * Calls poncho's changeDirection event if certain conditions are met
	 * @param {({'up'|'down'|'left'|'right'})} direction
	 */
	changeDirection(direction) {
		if (this.allowKeyPresses && this.gameEngine.running) {
			this.poncho.changeDirection(direction, this.allowponchoMovement);
		}
	}
	...
	/**
	 * Animates poncho's death, subtracts a life, and resets character positions if
	...
	deathSequence() {
		...
		new Timer(() => {
			...
			this.poncho.prepDeathAnimation();
			if (isSafari()) {
				playSound('death');
			} else {
	...
	advanceLevel() {
		...
		const imgBase = 'app/ext/poncho-games-login/resources/graphics/spriteSheets/maze/';
	...
	displayText(position, amount, duration, width, height) {
		...
		pointsDiv.style.backgroundImage =
			'url(app/ext/poncho-games-login/resources/graphics/' + `spriteSheets/text/${amount}.svg`;
...
class Pickup {
	constructor(type, scaledTileSize, column, row, poncho, mazeDiv, points) {
		this.type = type;
		this.poncho = poncho;
		this.mazeDiv = mazeDiv;
		this.points = points;
		this.nearponcho = false;
	...
	determineImage(type, points) {
		...
		return `url(app/ext/poncho-games-login/resources/graphics/spriteSheets/pickups/${image}.svg)`;
	}
	...
	/**
	 * Makes the fruit invisible (happens if poncho was too slow)
	 */
	hideFruit() {
	...
	/**
	 * Returns true if the Pickup is touching a bounding box at poncho's center
	 * @param {({ x: number, y: number, size: number})} pickup
	 * @param {({ x: number, y: number, size: number})} originalponcho
	 */
	checkForCollision(pickup, originalponcho) {
		const poncho = Object.assign({}, originalponcho);

		poncho.x += poncho.size * 0.25;
		poncho.y += poncho.size * 0.25;
		poncho.size /= 2;

		return (
			pickup.x < poncho.x + poncho.size &&
			pickup.x + pickup.size > poncho.x &&
			pickup.y < poncho.y + poncho.size &&
			pickup.y + pickup.size > poncho.y
		);
	}

	/**
	 * Checks to see if the pickup is close enough to poncho to be considered for collision detection
	 * @param {number} maxDistance - The maximum distance poncho can travel per cycle
	 * @param {({ x:number, y:number })} ponchoCenter - The center of poncho's hitbox
	 * @param {Boolean} debugging - Flag to change the appearance of pickups for testing
	 */
	checkponchoProximity(maxDistance, ponchoCenter, debugging) {
		if (this.animationTarget.style.visibility !== 'hidden') {
			const distance = Math.sqrt(
				(this.center.x - ponchoCenter.x) ** 2 +
					(this.center.y - ponchoCenter.y) ** 2
			);

			this.nearponcho = distance <= maxDistance;

			if (debugging) {
				this.animationTarget.style.background = this.nearponcho
					? 'lime'
					: 'red';
			}
		}
	...
	/**
	 * Checks if the pickup is visible and close to poncho
	 * @returns {Boolean}
	 */
	shouldCheckForCollision() {
		return (
			this.animationTarget.style.visibility !== 'hidden' && this.nearponcho
		);
	}

	/**
	 * If the Pickup is still visible, it checks to see if it is colliding with poncho.
	 * It will turn itself invisible and cease collision-detection after the first
	 * collision with poncho.
	 */
	update() {
		if (this.shouldCheckForCollision()) {
			if (
				this.checkForCollision(
					...
					{
						x: this.poncho.position.left,
						y: this.poncho.position.top,
						size: this.poncho.measurement
					}
				)
			) {
				this.animationTarget.style.visibility = 'hidden';
...
class SoundManager {
	constructor() {
		this.baseUrl = 'app/ext/poncho-games-login/resources/audio/';
		this.fileFormat = 'mp3';
		this.masterVolume = 1;
		this.paused = false;
		this.cutscene = true;
